...
const JobDetailPage = () => {
  ...
  useEffect(() => {
    const checkSavedStatus = async () => {
      if (isAuthenticated && role === 'candidate' && user?.id && job) { // Ensure job is loaded before checking saved status
        try {
          const response = await api.get(`/saved-jobs/${job.id}/status`); // Backend needed for this!
    ...
    if (!loading && job) { // Only check saved status once job is loaded
      checkSavedStatus();
    }
  }, [job, isAuthenticated, role, user, loading]); // Add job and loading to dependencies
